...
class BaseService(Generic[T]):
    ...
    def list(self, query_params: dict, tenant_id: str) -> dict:
        ...
        try:
            ...
            logger.debug(f"Listing records for tenant {tenant_id} in collection {collection} with params: {query_params}")
            ...
            logger.error(f"Failed to list records for tenant {tenant_id}: {str(e)}")
    ...
    def get(self, id: str, tenant_id: str, query_params: Optional[dict] = None) -> dict:
        ...
        try:
            collection = resolve_collection(self.collection_name, tenant_id)
            logger.debug(f"Fetching record {id} for tenant {tenant_id} in collection {collection} with params: {query_params}")
            record = self.pb.collection(collection).get_one(id, query_params)
            return sanitize_record(record)
        except Exception as e:
            logger.error(f"Failed to fetch record {id} for tenant {tenant_id}: {str(e)}")
            raise NotFoundException("Record not found")

    def create(self, data: T, tenant_id: str, query_params: Optional[dict] = None) -> dict:
        """Create a new record with optional expand and fields"""
        try:
            collection = resolve_collection(self.collection_name, tenant_id)
            logger.debug(f"Creating record for tenant {tenant_id} in collection {collection}: {data}")
            record = self.pb.collection(collection).create(data.model_dump(), query_params)
            return sanitize_record(record)
        except Exception as e:
            logger.error(f"Failed to create record for tenant {tenant_id}: {str(e)}")
    ...
    def update(self, id: str, data: T, tenant_id: str, query_params: Optional[dict] = None) -> dict:
        """Update an existing record with optional expand and fields"""
        try:
            collection = resolve_collection(self.collection_name, tenant_id)
            logger.debug(f"Updating record {id} for tenant {tenant_id} in collection {collection}: {data}")
            record = self.pb.collection(collection).update(id, data.model_dump(exclude_unset=True), query_params)
            return sanitize_record(record)
        except Exception as e:
            logger.error(f"Failed to update record {id} for tenant {tenant_id}: {str(e)}")
    ...
    def delete(self, id: str, tenant_id: str) -> dict:
        ...
        try:
            collection = resolve_collection(self.collection_name, tenant_id)
            logger.debug(f"Deleting record {id} for tenant {tenant_id} in collection {collection}")
            self.pb.collection(collection).delete(id)
            return {}
        except Exception as e:
            logger.error(f"Failed to delete record {id} for tenant {tenant_id}: {str(e)}")
            raise NotFoundException("Record not found")